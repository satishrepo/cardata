...
class Api extends CI_Controller {
  ...
  */
    ...
    if($id) {
      ...
      $value = $this->db->get_where("cars", ['id' => $id])->row_array();
      
      $response = "";

      if(!empty($value)) {

      	$response .= "<div class='row' link='".$value['url']."'>";
     		// $response .= "<div class='col'>".$value['id']."</div>";
     		// $response .= "<div class='col'>".$value['dealer']."</div>";
     		// $response .= "<div class='col-2'>".$value['url']."</div>";
     		$response .= "<div class='col'>".$value['title']."</div>";
     		$response .= "<div class='col-4'>".$value['blurb']."</div>";
     		$response .= "<div class='col'><img src='".$value['image']."' height='50px'></div>";
     		// $response .= "<div class='col'>".$value['date']."</div>";
	     	// 	$response .= "<div class='col'>";
	     	// 	if($value['status'] == 1) {
		     // 			$response .= "<span class='badge badge-success'>Active</span>";
		     // 		} else {
		     // 			$response .= "<span class='badge badge-danger'>Inactive</span>";
		     // 		}
	   			// $response .= "</div>";
   			$response .= "</div>";
